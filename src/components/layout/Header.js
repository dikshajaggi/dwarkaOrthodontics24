    ...
    <>
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          {searchOpen && (
            <div ref={searchBarRef} className="pb-4 pt-1">
              <div className="relative">
                ...
                />
                {searchQuery.trim() && (
                  <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-slate-100 rounded-xl shadow-lg z-30 overflow-hidden">
                    {searchResults.length > 0 ? (
                      searchResults.map((product) => (
                        <button
                          key={product.id}
                          onMouseDown={() => handleSearchResultClick(product.slug)}
                          className="w-full text-left flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0"
                        >
                          <div className="flex-1 min-w-0">
                            <span className="text-sm text-slate-800 block truncate">{product.name}</span>
