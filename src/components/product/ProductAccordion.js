...
export default function ProductAccordion({ product }) {
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-2xl divide-y divide-slate-50 px-5">
        ...
        {specEntries.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 mt-1">
            {specEntries.map(([key, val]) => (
              <div key={key} className="flex justify-between gap-4 py-1 border-b border-slate-50">
                <span className="text-slate-500 text-xs uppercase tracking-wide shrink-0">
                  {key.replace(/([A-Z])/g, ' $1').trim()}
                </span>
                <span className="text-slate-900 text-sm font-medium text-right">{val}</span>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-slate-400 text-sm">Specifications not available.</p>
        )}
      </AccordionItem>
      ...
      {/* Packaging */}
      <AccordionItem title="Packaging Information">
        <p className="text-slate-600">{product.packaging}</p>
      </AccordionItem>
